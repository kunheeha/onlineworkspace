...
def createWorkspace(request):
    if request.method == 'POST':
        ...
        if request.user.profile.workspaces_number > 0:
            if form.is_valid():
                form.save()
                request.user.profile.workspaces_number -= 1
            ...
            messages.warning(request, 'You exceeded the number of workspaces you can create, please buy more to create more workspaces')
    ...
    if request.method == 'POST':
        ...
        if 'addtask' in request.POST:
            ...
            if createfolderform.is_valid():
                ...
                return redirect('user-workspace', workspace_id=workspace_id)
        elif 'incompleteTask' in request.POST:
...
class WorkspaceInviteView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    model = Workspace
    template_name = 'workspace/workspace_invite.html'
...
def folder(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if 'uploadfile' in request.POST:
            if request.user.profile.files_number > 0:
                if uploadfileform.is_valid():
                    uploadfileform.save()
                    request.user.profile.files_number -= 1
                    request.user.profile.save()
                    return redirect('user-folder', workspace_id=workspace_id, folder_id=folder_id)
            else:
                messages.warning(request, 'You exceeded the number of files you can upload, please buy more to upload more files')
                return redirect('profile')
